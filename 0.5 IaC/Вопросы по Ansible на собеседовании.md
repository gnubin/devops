...
Это режим работы Ansible когда запрос к серверу выполняется напрямую из командной строки, без создания дополнительных файлов.
...
В Ansible есть идемпотентность в модулях. Если я скажу создать файл при первом запуске он его создаст. При втором он увидит, что создано, и создавать не будет.
...
### 4. Сценарий (Play) или Плейбук (Playbook)
...
***
### В чем разница между модулем и плагином в Ansible?

Модули что-то делают на хостах. Модули вызываются в задачах (tasks) плейбука
Плагины расширяют функционал Ansibleа
***
