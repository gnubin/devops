...
Можно ли собрать Docker-образ без Dockerfile? Если да, то как и каковы недостатки этого метода? 
Что выведет команда docker build для приведенного Dockerfile?   
Напишите пример Dockerfile, использующего инструкцию ARG.  
Для чего используется инструкция EXPOSE в Dockerfile? Открывает ли она порт на хосте?   
Может ли злоумышленник внутри контейнера изменить системное время хоста? От чего это зависит?   
Под каким пользователем по умолчанию запускаются процессы внутри контейнера? Почему это важно с точки зрения безопасности?   
Опишите ваш опыт работы с параметрами защиты контейнеров (запуск от непривилегированного пользователя, AppArmor, SELinux).   
Перечислите best practices для написания Dockerfile.   
Опишите ваш опыт работы с Docker: написание Dockerfile с нуля, упаковка приложений в контейнеры.   
Как устроена файловая система контейнеров (слои, Copy-on-Write)? 
***
...
***
### В чем разница написания Dockerfile для Java и Go приложений
